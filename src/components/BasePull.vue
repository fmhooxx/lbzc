...
export default {
    ...
    async mounted() {
        // if (this.$listeners.pull && this.$platform() != 'h5') {
    ...
    methods: {
        async pullInit() {
            await this.$nextTick(() => {
                ...
                this.pullIconHeight = this.$refs.refresh.clientHeight; // 刷新图标和文字提示
        ...
        touchmove(event) {
            ...
            if (window.scrollY == 0 && this.initY) {
                ...
                this.content.style.transform = `translateY(${this.height}px)  translateZ(0)`;
            }
        },
        touchend(event) {
            if (this.isRequest) return;
        ...
        toStay() {
            this.isRequest = true;
            this.content.style.transition = 'all 0.1s linear';
            this.height = this.stayY;
            this.content.style.transform = `translateY(${this.height}px)  translateZ(0)`;
        },
        toInit() {
            this.content.style.transition = 'all 0.3s cubic-bezier(1, 0.03, 0.58, 1)';
            this.height = 0;
            this.content.style.transform = `translateY(${this.height}px)  translateZ(0)`;
        },
